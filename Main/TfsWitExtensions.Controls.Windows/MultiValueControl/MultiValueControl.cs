...
namespace TfsWitExtensions.Controls.Windows
{
    public sealed partial class MultiValueControl : UserControl, IWorkItemControl, IWorkItemToolTip
    {
        ...
        private void SetAutoCompleteCheckedItem(string text) {
            ...
            m_multiValueCombo.CheckedListBoxAutoComplete.Items.Clear();
        ...
        public List<string> GetData()
        {
            ...
            if (this.IsDataSourceDefined())
            {
                ...
                //We transform without a json deserializer to avoid dependencies. 
                if (!String.IsNullOrWhiteSpace(result) && result.StartsWith("[\"") && result.EndsWith("\"]"))
                {
                    result = result.Substring(2, result.Length - 4);
                }

                var splittedResult = result.Split(new string[] { "\",\"" }, StringSplitOptions.RemoveEmptyEntries);

                for (int i = 0; i < splittedResult.Length; i++)
                {
                    if (!String.IsNullOrWhiteSpace(splittedResult[i].Trim()))
                    {
                        coll.Add(splittedResult[i].Trim());
                    }
                }
            }
            return coll;
